    ...
    return (
      ...
      >
        ...
        <header className="sticky top-0 z-50 bg-white px-4 py-3 flex items-center gap-3 border-b border-outline-variant/10 shadow-sm">
          ...
          <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-[16px] bg-primary/10 text-primary uppercase overflow-hidden">
      ...
      <div className="space-y-4 pt-1">
        <div className="divide-y divide-outline-variant/10">
          {conversations.map((chat) => (
            ...
            >
              {/* Avatar */}
              <div className="w-12 h-12 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-[18px] flex-shrink-0 uppercase overflow-hidden">
              ...
              <div className="flex-grow min-w-0">
                <div className="flex justify-between items-baseline mb-1">
                  <h3 className="font-bold text-[15px] text-on-surface truncate">{chat.customerId?.name}</h3>
                  <span className={`text-[11px] whitespace-nowrap ml-2 ${chat.unreadByVendor > 0 ? 'text-primary font-bold' : 'text-on-surface-variant'}`}>
                    {formatTime(chat.lastMessageAt)}
                  </span>
                </div>
                <p className={`text-[13px] truncate ${chat.unreadByVendor > 0 ? 'text-on-surface font-semibold' : 'text-on-surface-variant'}`}>
                  {chat.lastMessageBy === 'vendor' ? 'You: ' : ''}{chat.lastMessage || 'Start a conversation...'}
                </p>
              </div>
