...
export default function BottomNavBar() {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-50 flex justify-center px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pointer-events-none">
      <div className="glass-nav pointer-events-auto w-full max-w-[420px] rounded-[1.75rem] flex justify-around items-center h-16 px-2">
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.label}
              to={item.path}
              className={`flex flex-col items-center justify-center gap-0.5 transition-all duration-200 active:scale-90 cursor-pointer py-1.5 px-3.5 rounded-2xl ${
                isActive
